...
export default function EmailVerificationBanner() {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('email-verification-required', handleVerificationRequired);
  }, []);
  ...
  if (!user || dismissed || (user.isEmailVerified !== false && !forcedVisible)) {
    return null;
  }
    ...
    <div className="bg-yellow-50 dark:bg-yellow-900/20 border-b border-yellow-200 dark:border-yellow-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2.5 flex items-center gap-3 flex-wrap">
        ...
        <button
          ...
          aria-label="Dismiss"
          className="p-1 rounded hover:bg-yellow-100 dark:hover:bg-yellow-800/40 transition-colors flex-shrink-0"
        >
          <svg className="w-4 h-4 text-yellow-700 dark:text-yellow-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
