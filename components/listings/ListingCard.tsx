...
export function ListingCard({ listing, onClick }: ListingCardProps) {
  ...
  const formattedDate = new Date(listing.releaseDate).toLocaleDateString('en-US', {
    month: 'short',
    day: 'numeric',
  });
      ...
      <div className="space-y-2">
        ...
        {!listing.available && (
          <div className="rounded-lg bg-red-50 px-3 py-1 text-center dark:bg-red-900/20">
            <span className="text-xs font-medium text-red-800 dark:text-red-400">Unavailable</span>
          </div>
        )}
      </div>
    </Card>
